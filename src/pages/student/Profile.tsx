...
const Profile = () => {
  ...
  const loadProfile = async () => {
    try {
      ...
      const {
        data: enrollmentData
      } = await supabase.from('enrollments').select('student_code').eq('student_id', user.id).order('created_at', {
        ascending: false
      }).limit(1).maybeSingle();
      setProfile({
  ...
  const loadKYCDocuments = async () => {
    try {
      ...
      const {
        data,
        error
      } = await supabase.storage.from('kyc-documents').list(user.id);
      if (error) throw error;
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const {
        error
      } = await supabase.storage.from('kyc-documents').upload(filePath, file);
      if (error) throw error;
      toast({
        title: 'Success',
  ...
  const handleDeleteDocument = async (fileName: string) => {
    try {
      ...
      const {
        error
      } = await supabase.storage.from('kyc-documents').remove([`${user.id}/${fileName}`]);
      if (error) throw error;
      toast({
        title: 'Success',
        ...
        <div className="max-w-2xl mx-auto space-y-6">
              ...
              <form onSubmit={handleUpdate} className="space-y-6">
                ...
                <div className="flex flex-col items-center gap-4 pb-6 border-b">
                  <div className="relative group">
                    ...
                    <button type="button" onClick={() => avatarInputRef.current?.click()} disabled={uploadingAvatar} className="absolute inset-0 flex items-center justify-center bg-black/50 rounded-full opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer">
                  ...
                  <div className="text-center">
                    ...
                    {profile.student_code && <div className="flex items-center justify-center gap-1 text-sm text-primary font-mono mt-1">
                ...
                {/* Student ID Card */}
                {profile.student_code && <div className="p-4 bg-primary/5 border border-primary/20 rounded-lg">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <p className="text-sm text-muted-foreground">Upload your identity documents for verification (Aadhaar, Passport-Size photo)</p>

              <div className="border-2 border-dashed rounded-lg p-6 text-center">
                <input type="file" id="kyc-upload" className="hidden" accept=".pdf,.jpg,.jpeg,.png" onChange={handleFileUpload} disabled={uploading} />
              ...
              {kycDocuments.length > 0 && <div className="space-y-2">
                  <Label>Uploaded Documents</Label>
                  <div className="space-y-2">
                    {kycDocuments.map(doc => <div key={doc.name} className="flex items-center justify-between p-3 bg-muted rounded-lg">
                        <button onClick={() => setPreviewDoc(doc)} className="flex items-center gap-3 flex-1 text-left hover:opacity-80 transition-opacity">
                        ...
                        <Button variant="ghost" size="icon" onClick={() => handleDeleteDocument(doc.name)} className="text-destructive hover:text-destructive ml-2">
              ...
              <Dialog open={!!previewDoc} onOpenChange={open => !open && setPreviewDoc(null)}>
                <DialogContent className="max-w-3xl max-h-[90vh]">
                  ...
                  <div className="flex items-center justify-center overflow-auto max-h-[70vh]">
                    {previewDoc && (previewDoc.name.toLowerCase().endsWith('.pdf') ? <iframe src={previewDoc.url} className="w-full h-[60vh] border rounded" title="Document Preview" /> : <img src={previewDoc.url} alt="Document Preview" className="max-w-full max-h-[60vh] object-contain rounded" />)}
                  </div>
                </DialogContent>
              </Dialog>
            </CardContent>
