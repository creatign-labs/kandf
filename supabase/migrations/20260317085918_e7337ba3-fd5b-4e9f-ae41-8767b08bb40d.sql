...
BEGIN
  -- If no recipe specified, get the next eligible recipe from eligibility check
  IF p_recipe_id IS NULL THEN
    SELECT elig.next_recipe_id INTO v_effective_recipe_id
    FROM check_student_booking_eligibility(p_student_id) elig
    WHERE elig.is_eligible = true;
    
    -- If eligible but no next recipe (all done), just check basic eligibility
    IF v_effective_recipe_id IS NULL THEN
      IF NOT EXISTS (
        SELECT 1 FROM check_student_booking_eligibility(p_student_id) elig
        WHERE elig.is_eligible = true
      ) THEN
        RETURN QUERY SELECT false, 'Not eligible to book'::text, NULL::uuid, NULL::uuid;
        RETURN;
      END IF;
    END IF;
  ELSE
    -- Recipe specified: check eligibility matches
    IF NOT EXISTS (
      SELECT 1 FROM check_student_booking_eligibility(p_student_id) elig
  ...
  -- Create booking record (recipe will be assigned by admin later if not specified)
