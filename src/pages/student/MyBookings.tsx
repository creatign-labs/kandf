...
import { Calendar as CalendarIcon, Clock, Users, AlertCircle, Loader2, X, Plus, History } from "lucide-react";
...
const MyBookings = () => {
  ...
  const selectedBatch = batches?.find(b => b.id === selectedBatchId);
  const hasExistingBooking = existingBookingsForDate && existingBookingsForDate.length > 0;
  ...
  const canCancel = (bookingDate: string) => {
    ...
    return date >= tomorrowCheck;
  };

  const isLoading = batchesLoading || bookingsLoading;
