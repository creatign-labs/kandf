CREATE OR REPLACE FUNCTION public.date_matches_course_days(p_date date, p_days text[])
RETURNS boolean
LANGUAGE plpgsql
IMMUTABLE
...
BEGIN
  ...
  FOREACH v_item IN ARRAY p_days LOOP
    v_item_l := lower(trim(v_item));
    IF v_item_l = '' THEN CONTINUE; END IF;
    IF v_item_l = v_day THEN RETURN true; END IF;
    IF v_item_l LIKE v_short || '%' THEN RETURN true; END IF;
    IF v_day LIKE v_item_l || '%' THEN RETURN true; END IF;
  END LOOP;
  RETURN false;
END;
$$;
...
BEGIN
  ...
  -- Enforce course is the same as enrollment course (course is locked at enrollment)
  IF v_enrollment.course_id <> p_course_id THEN
    RETURN QUERY SELECT false, 'You can only book slots for your enrolled course'::text, NULL::uuid, NULL::uuid;
    RETURN;
  END IF;
  ...
  IF NOT FOUND THEN
    RETURN QUERY SELECT false, 'Course not found'::text, NULL::uuid, NULL::uuid;
    RETURN;
  END IF;
  ...
  IF v_course.days_of_week IS NOT NULL
    ...
    RETURN QUERY SELECT false, 'Selected date is not available for this course'::text, NULL::uuid, NULL::uuid;
    RETURN;
  END IF;
  ...
  WHERE b.course_id = p_course_id
    ...
    AND (b.end_date IS NULL OR b.end_date >= p_batch_date)
  ...
  -- Sum capacity across all matching batches for this course/time/date
  SELECT COALESCE(SUM(b.total_seats), 0) INTO v_total_seats
  FROM batches b
  WHERE b.course_id = p_course_id
    AND b.time_slot = p_time_slot
    AND b.booking_enabled = true
  ...
  IF v_total_seats > 0 AND v_confirmed_count >= v_total_seats THEN
    RETURN QUERY SELECT false, 'Slots for this batch are full. Please contact admin.'::text, NULL::uuid, NULL::uuid;
    RETURN;
  END IF;
